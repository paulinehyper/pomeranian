...
# 앱 시작 시 메일 연동 안내 및 환경설정 창 자동 표시
...
def show_settings_dialog():
    root = tk.Tk()
    root.withdraw()
    messagebox.showinfo("메일 연동 필요", "메일을 연동하세요")
    root.deiconify()  # 팝업 확인 후 루트 창을 잠깐 보이게
    center_window(root)  # root 창을 화면 중앙에 배치
    SettingsDialog(root, load_settings())
    root.mainloop()
...
class SettingsDialog:
    ...
    def __init__(self, parent, current_settings):
        ...
        # geometry는 center_window에서만 설정
        self.dialog.resizable(False, False)
        self.dialog.transient(parent)
        self.dialog.grab_set()

        # Variables
        self.mail_server_var = tk.StringVar(value=current_settings.get("mail_server", "Gmail"))
    ...
    def save(self):
        ...
        save_settings(self.result)  # 파일에 저장
        self.dialog.destroy()
        try:
...
class TodoApp:
    ...
    def setup_tray_icon(self):
        ...
        icon_path = resource_path("img.png")
...
def main():
    ...
    if not username or not password:
        ...
        center_window(root)
        from tkinter import messagebox
        messagebox.showinfo("메일 연동정보가 없습니다", "환경설정에서 메일 정보를 입력해야 메일 기능을 사용할 수 있습니다.")
        root.destroy()
        show_settings_dialog()
        # 환경설정 창 닫힌 후 다시 값 확인
        settings = load_settings()
...
if __name__ == "__main__":
>>>>>>> 272c865ef99ebf14f15480398942c2fbdc01524f
    main()